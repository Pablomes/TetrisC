...
#define RENDERER "color/renderer.h"
...
mainMenu:
    ...
    int memRes = initialiseMem(gameHeight, gameWidth, 7500);
...
startGame:
    ...
    while (running) {
        ...
        if (spawn == 1) {
            ...
            if (drop == 1 || drop == -1) {
                res = updatePiece(glob_piece, dir, drop, glob_board, gameHeight, gameWidth, (int)glob_piece->type + 1);
                drop = 0;
            } else {
                res = updatePiece(glob_piece, dir, 0, glob_board, gameHeight, gameWidth, (int)glob_piece->type + 1);
            }
        ...
        if (spawn2 == 1 && twoPlayer) {
            ...
            if (drop2 == 1 || drop2 == -1) {
                res = updatePiece(piece2, dir2, drop2, board2, gameHeight, gameWidth, (int)glob_piece->type + 1);
                drop = 0;
            } else {
                res = updatePiece(piece2, dir2, 0, board2, gameHeight, gameWidth, (int)glob_piece->type + 1);
            }
