...
int initialiseGamePlayer2(int height, int width) {
    ...
    memset(board2, 0, sizeof(board2[0])*width*height);
...
int spawnPiece(Piece* piece, int* board, PieceType* pieces, int hold, int height, int width, int player) {
    ...
    switch (type) {
        ...
        case RS_BLOCK:
        case SQUARE:
            if (board[1*width + width / 2] != 0 || board[1*width + width / 2 + 1] != 0 || board[1*width + width / 2 - 1] != 0) {
                movePiece(piece, width / 2, 0);
            } else {
    ...
    switch (piece->type) {
        case H_LINE:
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (yPos >= height || yPos < 0 || xPos >= width || xPos < 0 || board[yPos*width + xPos] != 0) {
...
int checkBoard(int* board, int height, int width) {
    ...
    while (row - cleared >= 0) {
        ...
        for (int i = 0; i < width; i++) {
            board[row*width + i] = board[(row - cleared)*width + i];
            if (board[row*width + i] == 0) {
                clear = 0;
            }
        }
...
int addRubbish(int* board, int amount, int height, int width, int rubbishVal) {
    ...
    for (int y = 0; y < height - amount; y++) {
        for (int x = 0; x < width; x++) {
            board[y*width + x] = board[(y + amount)*width + x];
        }
    }
